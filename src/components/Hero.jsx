...
import { getHeroImage } from '../services/productService'
import defaultHeroImage from '../assets/hero-perfume.jpg'
...
const Hero = () => {
    ...
    useEffect(() => {
        getHeroImage().then(url => {
            setHeroImg(url || defaultHeroImage)
            setIsLoading(false)
        }).catch(() => {
        ...
        <section className="relative w-full min-h-screen flex items-center bg-[#fdfdf1] overflow-hidden pt-20">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                ...
                <motion.div
                    ...
                    // Agregamos flex y centrado aquí para mantener alineada la imagen más pequeña
                    className="relative z-10 w-full flex justify-center mt-10 md:mt-0"
                >

                    {/* NUEVO: Contenedor que limita el ancho máximo en pantallas grandes */}
                    <div className="relative w-full max-w-[280px] sm:max-w-[340px] md:max-w-[340px] lg:max-w-[400px] xl:max-w-[420px]">

                        <div className="relative w-full aspect-[4/5] rounded-[2rem] overflow-hidden shadow-2xl bg-[#f0f0e8]">
                            {isLoading ? (
                                <div className="w-full h-full bg-[#e5e5dd] animate-pulse" />
                            ) : (
                                <img
                                    src={heroImg}
                        ...
                        {!isLoading && (
                            <motion.div
                                ...
                                // Ajusté el "left" para que el badge no se desborde demasiado ahora que la imagen es más chica
                                className="absolute -bottom-6 -left-4 sm:-left-8 bg-[#fdfdf1]/90 backdrop-blur-sm p-5 md:p-6 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] max-w-[200px] hidden md:block border border-[#2b323f]/5 z-20"
                            >
